...
namespace XTLAB
{
    ...
    public class MyListTagHelper : TagHelper
    {
            ...
            public override void Process(TagHelperContext context, TagHelperOutput output)
            {
                  ...
                  output.TagMode = TagMode.StartTagAndEndTag;   // Có đóng thẻ ul

                  output.Attributes.SetAttribute("class", "list-group");
                  output.PreElement.AppendHtml($"<h2>{ListTitle}</h2>");    // Chèn vào phía trước phần tử ul là dòng h2


                  StringBuilder content = new StringBuilder();  
                  foreach (var item in ListItems)
                  {
                      content.Append($@"<li class=""list-group-item"">{item}</li>");    // các phần tử li
                  }
